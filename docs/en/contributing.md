...
- **Editing the docs**: every human doc is bilingual — update both `docs/en/` and `docs/es/` in the same change. The agent role files, `templates/`, and the session baseline stay English (the canonical machine layer).
